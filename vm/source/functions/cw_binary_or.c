...
static void		cw_execute_or(t_command *cmd, t_processes *proc)
{
...
static int		cw_get_args_av_or(t_command *cmd, t_processes *proc)
{
...
void			cw_binary_or(t_processes *proc)
{
	...
	if (cw_get_args_av_or(&cmd, proc) && cw_is_valid_reg(&cmd))
		cw_execute_or(&cmd, proc);
