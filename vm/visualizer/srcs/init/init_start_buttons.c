...
bool				init_start_buttons(t_arena *arena)
{
	...
	if (!(arena->start_btns[START_MENU_BTN] =
			create_button(0, (SDL_Point){.x = wd, .y = 150 + BUTTON_HEIGHT},
							"START", arena->renderer,
							(SDL_Point){.x=BUTTON_WIDTH, .y=BUTTON_HEIGHT})) ||
		!(arena->start_btns[STOP_MENU_BTN] =
			create_button(0, (SDL_Point){.x = wd, .y = 300 + BUTTON_HEIGHT},
							"STOP", arena->renderer,
							(SDL_Point){.x=BUTTON_WIDTH, .y=BUTTON_HEIGHT})) ||
		!(arena->start_btns[INFO_MENU_BTN] =
			create_button(0, (SDL_Point){.x = wd, .y = 450 + BUTTON_HEIGHT},
							"INFO", arena->renderer,
							(SDL_Point){.x=BUTTON_WIDTH, .y=BUTTON_HEIGHT})) ||
		!(arena->start_btns[EXIT_MENU_BTN] =
			create_button(0, (SDL_Point){.x = wd, .y = 600 + BUTTON_HEIGHT},
							"EXIT", arena->renderer,
							(SDL_Point){.x=BUTTON_WIDTH, .y=BUTTON_HEIGHT})))
		return (false);
	return (true);
